...
export type AIModel = "gpt-4-turbo-preview" | "gpt-3.5-turbo" | "claude-3-opus" | "claude-3-sonnet";
...
export class AIService {
    ...
    constructor(provider: AIProvider = "openai", model?: AIModel) {
        ...
        this.model = model || (provider === "openai" ? "gpt-4-turbo-preview" : "claude-3-sonnet");
    }
    ...
    async generateCompletion(
        ...
        if (this.provider === "openai") {
            ...
            return this.generateAnthropicCompletion(systemPrompt, userPrompt, options);
        } else {
    ...
    private async generateAnthropicCompletion(
        ...
        const estimatedTokens = Math.ceil((systemPrompt.length + userPrompt.length + content.text.length) / 4);
