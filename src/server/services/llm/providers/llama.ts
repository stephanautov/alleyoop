...
export class LlamaProvider implements LLMProvider {
  ...
  constructor(config: LlamaProviderConfig = {}) {
    ...
    switch (this.provider) {
      ...
      case "local":
        this.baseUrl =
          config.baseUrl || env.LOCAL_LLAMA_URL || "http://localhost:8080";
        break;
    }
  }
  ...
  private async generateWithReplicate(
    ...
    try {
      ...
      const model =
        ...
        params.model ||
        "meta/meta-llama-3-70b-instruct:latest";

      // Format prompt for Llama chat format
      const formattedPrompt = this.formatLlamaPrompt(
        params.systemPrompt,
        params.prompt,
      );
  ...
  private async generateWithOpenAICompatible(
    ...
    try {
      ...
      return {
        content,
        promptTokens:
          data.usage?.prompt_tokens || this.countTokens(params.prompt),
        completionTokens:
          data.usage?.completion_tokens || this.countTokens(content),
  ...
  private async generateWithLocal(
    ...
    try {
      ...
      const completionTokens =
        data.tokens_predicted || this.countTokens(content);
  ...
  estimateCost(tokens: number, model: string): number {
    ...
    const pricing: Record<string, { input: number; output: number }> = {
      ...
      "togethercomputer/llama-2-7b-chat": { input: 0.2, output: 0.2 },
      "togethercomputer/llama-2-13b-chat": { input: 0.225, output: 0.225 },
      "togethercomputer/llama-2-70b-chat": { input: 0.9, output: 0.9 },
    ...
    const modelPricing = pricing[model] || { input: 0.5, output: 0.5 };
  ...
  async generateCode(params: {
    ...
    return this.generateCompletion({
      ...
      systemPrompt:
        "You are CodeLlama, an AI assistant specialized in generating high-quality code.",
    });
  }
}
