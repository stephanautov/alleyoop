...
async function validateRouter(
    ...
    if (options.model) {
        ...
        try {
            ...
            if (!schema.includes(`model ${options.model}`)) {
                result.warnings.push(`Model "${options.model}" not found in Prisma schema`);
                result.suggestions.push(`Run "npm run db:push" after creating the model`);
            }
        } catch {
    ...
    if (name.includes("_") || name.includes("-")) {
        result.suggestions.push("Use camelCase for router names (e.g., userProfile instead of user_profile)");
    }
}
...
async function validateComponent(
    ...
    if (name[0] !== name[0].toUpperCase()) {
        ...
        result.suggestions.push(`Consider renaming to ${name[0].toUpperCase() + name.slice(1)}`);
    }

    // Check if component directory already exists
    const baseDir = options.type === "page" ? "src/app" : "src/components";
    const componentDir = path.join(process.cwd(), baseDir, options.dir || "", name);

    if (await fileExists(componentDir)) {
        const files = await fs.readdir(componentDir);
        if (files.length > 0) {
            result.conflicts.push(`Component directory already exists with ${files.length} files`);
            result.valid = false;
        }
    }
}
...
async function validateFeature(
    ...
    if (options.includeApi && !options.model) {
        result.suggestions.push("Consider specifying a Prisma model for API generation");
    }
    ...
    if (options.includeTests) {
        ...
        try {
            ...
            result.warnings.push("Testing library not found. Run: npm install -D @testing-library/react");
        }
    }
}
