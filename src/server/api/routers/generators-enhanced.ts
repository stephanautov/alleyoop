...
export const enhancedGeneratorsRouter = createTRPCRouter({
  generateRouterWithValidation: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Rate limiting
      const { remaining } = await rateLimitMiddleware(
        ctx.session.user.id,
        "generate:router",
      );
      ...
      try {
        ...
        return {
          ...
          files:
            generatedFilesStore
              .get(sessionId)
              ?.map((f: GeneratedFileInfo) => f.path) || [],
        };
      } catch (error) {
  ...
  getMetrics: adminProcedure.query(async ({ ctx }) => {
    ...
    const summary = metrics.reduce(
      (acc: Record<string, any>, metric: any) => {
    ...
    return {
      ...
      successRate:
        metrics.length > 0
          ? metrics.filter((m: any) => !m.failed).length / metrics.length
          : 0,
    };
  }),
});
