...
export async function formatCode(code: string): Promise<string> {
    try {
        ...
        const { stdout } = await execAsync(`echo '${code.replace(/'/g, "\\'")}' | npx prettier --stdin-filepath file.ts`);
        return stdout;
    } catch (error) {
 ...
 */
export function generateInterface(name: string, obj: Record<string, any>): string {
 ...
 */
export function parseZodSchema(schemaString: string): Array<{ name: string; type: string; optional: boolean }> {
